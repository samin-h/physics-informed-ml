...
class AttentionBottleneck(nn.Module):
    ...
    def __call__(self, z: jnp.ndarray) -> jnp.ndarray:
        ...
        queries = self.param(
            "source_queries", nn.initializers.normal(0.02), (self.n_sources, d)
        )
        ...
        context = jnp.broadcast_to(context[:, :, None, :], (B, self.n_sources, T, d))
        z_expanded = jnp.broadcast_to(z[:, None, :, :], (B, self.n_sources, T, d))
...
class GWSeparator(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        """
        # Encode mixture
        z = ComplexEncoder(hidden_dims=self.encoder_dims, latent_dim=self.latent_dim)(x)
        # z: (B, T, latent_dim)

        # Separate into N source tracks via attention
        tracks = AttentionBottleneck(
            n_sources=self.n_sources, latent_dim=self.latent_dim, n_heads=self.n_heads
        )(z)
        ...
        masks_flat = ComplexDecoder(
            hidden_dims=self.decoder_dims, out_dim=self.n_freqs
        )(tracks_flat)
        # masks_flat: (B*N, T, F)
