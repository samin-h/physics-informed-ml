...
def create_batches(mixture, h1, h2, params1, params2, batch_size: int = 80) -> tuple[jnp.ndarray, ...]:
...
def process_data(mixture_batch, h1_batch, h2_batch) -> tuple[jnp.ndarray, ...]:
    ...
    for i in range(N):
        MIX_B, H1_B, H2_B, PSD_B = jax.jit(jax.vmap(preprocess_sample_jax, in_axes=(0, 0, 0), out_axes=(0, 0, 0, 0)))(mixture_batch[i], h1_batch[i], h2_batch[i])
...
def batch_iterator(
    ...
    for s_path in signal_paths:
        ...
        mixture, h1, h2, params1, params2 = load_shard_only(signal_path = s_path, params_path = p_path)
        # print(f"Finished Loading: {s_path.name}")
        
        # print(f"Creating Batches:  {s_path}")
        mix_b, h1_b, h2_b, params1_b, params2_b= create_batches(mixture, h1, h2, params1, params2, batch_size=batch_size)
        ...
        for batch_idx in batch_indices:
            batch = {
                "mixture" : MIX_b[batch_idx],
                "h1"      : H1_b[batch_idx],
                "h2"      : H2_b[batch_idx],
                "psd"     : PSD_b[batch_idx],
                "params1" : params1_b[batch_idx],
                "params2" : params2_b[batch_idx]
            }
            # t1 = perf_counter()
...
def get_shard_splits(data_dir: str,
                     ...
                     val_frac : float = 0.1) -> tuple[list[Path], ...]:
    """
